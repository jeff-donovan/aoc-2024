...
def num_design_combos(combo_cache, towels, remaining_design):
    if remaining_design == '':
        return 1

    if remaining_design in combo_cache:
    ...
    if len(remaining_designs_to_check) == 0:
        combo_cache[remaining_design] = 0
        return 0

    # at this point we know a towel starts off the remaining design so we can check the truncated designs
    combo_cache[remaining_design] = sum([num_design_combos(combo_cache, towels, next_remaining_design) for next_remaining_design in remaining_designs_to_check])
    return combo_cache[remaining_design]
...
if __name__ == '__main__':
    ...
    cache = {}
    print(sum([num_design_combos(cache, towels, d) for d in designs]))
